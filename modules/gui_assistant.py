...
rec_email, rec_phoneno = "mini51019158@gmail.com", ""
WAEMEntry = None
...
try:
	...
	ownerDesignation = "Sir" or "My dear"
...
def record(clearChat=True, iconDisplay=True):
	...
	while True:
		...
		if isContain(query, EXIT_COMMANDS): 
			speak("Shutting down... Goodbye!",True,True) 
			break
...
def main(text):
		...
		if "open edge" in text:
			speak("Opening MS Edge...", True, True)
			os.startfile('C:\\Program Files (x86)\\Microsoft\\Edge\\Application\\msedge.exe')
			return
		if "open word" in text:
			speak("Opening MS Words...", True, True)
			os.startfile('C:\\Program Files (x86)\\Microsoft Office\\root\\Office16\\WINWORD.EXE')
			return
		if "open powerpoint" in text:
			speak("Opening MS PowerPoint...", True, True)
			os.startfile('C:\\Program Files (x86)\\Microsoft Office\\root\\Office16\\POWERPNT.EXE')
			return
		if "open excel" in text:
			speak("Opening MS Excel...", True, True)
			os.startfile('C:\\Program Files (x86)\\Microsoft Office\\root\\Office16\\EXCEL.EXE')
			return
		if "open setting" in text:
			speak("Opening Settings...", True, True)
			os.startfile('C:\\Windows\\System32\\control.exe')
			return
		if "open swindow" in text:
			speak("Searching...", True, True)
			os.startfile('D:\\Do not open\\windows files\\Shortcut files\\Search')
			return
		if "open task view" in text:
			speak("Task viewing...", True, True)
			os.startfile('D:\\Do not open\\windows files\\Shortcut files\\Task view')
			return
		if "open evkey" in text:
			speak("Opening Evkey", True, True)
			os.startfile('C:\\Program Files (x86)\\EvkeyVN\\EVKey32.exe')
			return
		if "help" in text:
			speak("Please read the base path...", True, True)
			os.startfile('PersonalAssistantChatbot-master\\README.md')
			return
		if "license" in text:
			speak("Here is my LICENSE", True, True)
			os.startfile('C:\\Users\\TTC\\Documents\\Code\\PersonalAssistantChatbot-master\\LICENSE')
			return
		if "open visual studio" in text:
			speak("Opening Visual Studio Code...", True, True)
			os.startfile('C:\\Users\\TTC\\AppData\\Local\\Programs\\Microsoft VS Code\\Code.exe')
			return
		if "open zalo" in text:
			speak("Opening Zalo...", True, True)
			os.startfile('C:\\Program Files (x86)\\Zalo.exe')
			return
		if "open foxit reader" in text:
			speak("Opening PDF Foxit Reader...", True, True)
			os.startfile('C:\\Program Files (x86)\\Foxit Software\\Foxit Reader\\FoxitReader.exe')
			return
		if "open ultraviewer" in text:
			speak("Opening UltraViewer...", True, True)
			os.startfile('C:\\Program Files (x86)\\UltraViewer\\UltraViewer_Desktop.exe')
			return
		if "open sublime text" in text:
			speak("Opening Sublime Text 4...", True, True)
			os.startfile('C:\\Program Files (x86)\\Sublime Text 3\\sublime_text.exe')
			return
		if "open media player" in text:
			speak("Opening Media Player...", True, True)
			os.startfile('C:\\Program Files\\Windows Media Player\\wmplayer.exe')
			return
		if "turn cleartype" in text:
			speak("Turning ClearType...", True, True)
			os.startfile('C:\\Windows\\SysWOW64\\cttune.exe')
			return
		if "turn on display" in text:
			speak("Turning on Display Settings...", True, True)
			os.startfile('C:\\Windows\\SysWOW64\\DpiScaling.exe')
			return
		if "open charactor editor" in text:
			speak("Opening Charactor Editor...", True, True)
			os.startfile('C:\\Windows\\SysWOW64\\eudcedit.exe')
			return
		if "open file transfer" in text:
			speak("Opening Bluetooth File Transfer...", True, True)
			os.startfile('C:\\Windows\\SysWOW64\\fsquirt.exe')
			return
		if "open task manager" in text:
			speak("Opening Task Manager...", True, True)
			os.startfile('C:\\Windows\\SysWOW64\\LaunchTM.exe')
			return
		if "open system information" in text:
			speak("Opening System Information...", True, True)
			os.startfile('C:\\Windows\\SysWOW64\\msinfo32.exe')
			return
		if "open remote desktop connection" in text:
			speak("Opening Remote Desktop Connection...", True, True)
			os.startfile('C:\\Windows\\SysWOW64\\mstsc.exe')
			return
		if "open performance monitor" in text:
			speak("Opening Performance Monitor...", True, True)
			os.startfile('C:\\Windows\\SysWOW64\\perfmon.exe')
			return
		if "turn on quick assist" in text:
			speak("Turning on Quick Assist...", True, True)
			os.startfile('C:\\Windows\\SysWOW64\\quickassist.exe')
			return
		if "open resource monitor" in text:
			speak("Opening Resource Monitor...", True, True)
			os.startfile('C:\\Windows\\SysWOW64\\resmon.exe')
			return
		if "open clean manager" in text:
			speak("Opening Clean Manager...", True, True)
			os.startfile('C:\\Windows\\SysWOW64\\cleanmgr.exe')
			return
		if "open record" in text:
			speak("Opening Record...", True, True)
			os.startfile('C:\\Windows\\SysWOW64\\psr.exe')
			return
		if "open charactormap" in text:
			speak("Opening Charactor Map...", True, True)
			os.startfile('C:\\Windows\\SysWOW64\\charmap.exe')
			return
		if "open magnifier" in text:
			speak("Opening Magnifier...", True, True)
			os.startfile('C:\\Windows\\SysWOW64\\Magnify.exe')
			return
		if "open command" in text:
		...
		if result != "None": speak(result, True, True)
			...
			speak("I don't know anything about this. Do you want to search it on web?. If it's an application or website, please add 'open' before!. *TYPE HELP* for helping", True, True)
